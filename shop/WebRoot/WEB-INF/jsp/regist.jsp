...
<link rel="stylesheet" type="text/css"
	...
	<script>
		...
		function checkPhone() {
			// 获得文件框值:
			var phone = document.getElementById("UCellPhone").value;
			// 1.创建异步交互对象
			var xhr = createXmlHttp();
			// 2.设置监听
			xhr.onreadystatechange = function() {
			...
			// 3.打开连接
			xhr.open("GET",
					"${pageContext.request.contextPath}/user_findByPhone.action?time="
							+ new Date().getTime() + "&phone=" + phone,
					true);
			// 4.发送
			xhr.send(null);
		}
...
<body>
							...
							<tbody>
								...
								<tr>
									...
									<td><input type="text" name="phone" class="text"
										id="UCellPhone" onblur="checkPhone()" /> <span
										id="span2"></span><span><s:fielderror
												fieldName="phone" /></span>
								</tr>
								<tr>
									<th><span class="requiredField"></span>动态密码:</th>
									<td><input type="text" placeholder="验证码" class="text"
										id="Code" name="Code" /> <input type="button" id="send"
										name="send" value="获取验证码 " /></td>
								</tr>
	...
	<div class="container footer">
		...
		<div class="span24">
			<div class="copyright">Copyright © 2015-2020 网上商城 版权所有</div>
		</div>
	</div>
	<div id="_my97DP"
